...
# checking the input phrase
def askHostToSayPhrase(repeatText=None):
    question = repeatText if repeatText is not None else "Enter the word for your opponent to guess.\n"
    word = input(question)
    if type(word) == str and len(word) >= 3:
        return word.lower()
    else:
        return askHostToSayPhrase("Your word must be at least three letters long. Enter a new word.\n")
...
def guessLetter(repeatText=None):
    question = repeatText if repeatText is not None else "Guess a letter.\n"
    word = input(question)
    if type(word) == str and len(word) == 1:
        return word.lower()
    else:
        return guessLetter("You can only guess one letter at a time. Try again.\n")
